...
async def get_tenant_usage(
    ...
    usage = await StripeBillingService.get_tenant_usage_async(session, tenant_id)
...
async def get_estimated_bill(
    ...
    tenant_id = tenant_context["tenant_id"]
    usage = await StripeBillingService.get_tenant_usage_async(session, tenant_id)
    bill = StripeBillingService.calculate_estimated_bill(tenant_id, custom_usage=usage)
